...
def evaluate(args, model, tokenizer, prefix=""):
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        for i, example_index in enumerate(example_indices):
            ...
            output = [to_list(output[i]) for _, output in outputs.items()]
    ...
    if not args.version_2_with_negative:
        ...
        results = musique_evaluate(examples, predictions)
    # Unanswerable
    else:
        # results = eval_squad(os.path.join(args.data_dir, args.predict_file), output_prediction_file, output_null_log_odds_file,
        #                     args.null_score_diff_threshold)
        results = musique_evaluate_2(os.path.join(args.data_dir, args.predict_file), output_prediction_file, output_null_log_odds_file,
                            args.null_score_diff_threshold)
    return results
...
def load_and_cache_examples(args, tokenizer, evaluate=False, output_examples=False):
    ...
    if os.path.exists(cached_features_file) and not args.overwrite_cache and not output_examples:
        ...
        if not args.data_dir and ((evaluate and not args.predict_file) or (not evaluate and not args.train_file)):
            ...
            examples = MusiqueProcessor().get_examples_from_dataset(tfds_examples, evaluate=evaluate)
        else:
            # processor = SquadV2Processor() if args.version_2_with_negative else SquadV1Processor()
            processor = MusiqueProcessor() if args.version_2_with_negative else MusiqueProcessor()
        ...
        features, dataset = musique_convert_examples_to_features( 
            examples=examples,
            tokenizer=tokenizer,
