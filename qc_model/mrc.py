...
from transformers import (
    BertConfig, BertForSequenceClassification, BertForMultipleChoice, BertForQuestionAnswering, BertTokenizer,
    XLMConfig, XLMForSequenceClassification, XLMForMultipleChoice, XLMForQuestionAnswering, XLMTokenizer, 
    XLNetConfig, XLNetForSequenceClassification, XLNetForMultipleChoice, XLNetForQuestionAnswering, XLNetTokenizer,
    DistilBertConfig, DistilBertForSequenceClassification, DistilBertForMultipleChoice, DistilBertForQuestionAnswering, DistilBertTokenizer,
    AlbertConfig, AlbertForSequenceClassification, AlbertForMultipleChoice, AlbertForQuestionAnswering, AlbertTokenizer,
    DebertaV2Config, DebertaV2ForSequenceClassification, DebertaV2ForMultipleChoice, DebertaV2ForQuestionAnswering, DebertaV2Tokenizer,
...
def evaluate(
        args, 
        classifier, sequence_generator, parallel_generator, retriever, reader, 
        classifier_tokenizer, generator_tokenizer, retriever_tokenizer, reader_tokenizer
    ):
    ...
    for i, data in enumerate(dataset):
        ...
        feature = classifier_tokenizer.encode_plus(
            question, 
            add_special_tokens=True, 
            max_length = args.max_cls_length,
        )
        padding_length = args.max_cls_length - len(feature["input_ids"])
        feature["input_ids"] = feature["input_ids"] + ([0] * padding_length)
        feature["attention_mask"] = feature["attention_mask"] + ([0] * padding_length)

        assert len(feature["input_ids"]) == args.max_cls_length
        assert len(feature["attention_mask"]) == args.max_cls_length
        ...
        classification = logits.detach().cpu().numpy()
        ...
        clock = 0
        is_end = False
        while clock < 5 and not is_end:
            # Generator
            if clock == 0 and classification == 1:
                features = generator_tokenizer([question], return_tensors='pt').to(args.device)
                output = parallel_generator.generate(input_ids=features['input_ids'], 
                        attention_mask=features['attention_mask'],
                        max_length=args.max_gen_length)
                querys = generator_tokenizer.decode(output[0])
                clock += 2
                querys = [q.strip() for q in querys.replace("<s>", "").replace("</s>", "").replace("<pad>", "").split(";")]
            else:
                if clock != 0:
                    length = len(querys)
                    for q_idx in range(length):
                        question += " ; " + querys[q_idx] + " " + ans[-(length-q_idx)]
                features = generator_tokenizer([question], return_tensors='pt').to(args.device)
                output = sequence_generator.generate(input_ids=features['input_ids'], 
                        attention_mask=features['attention_mask'],
                        max_length=args.max_gen_length)
                query = generator_tokenizer.decode(output[0])
                if query.endswith(" </s>"):
                    is_end = True
                clock += 1
                query = query.replace("<s>", "").replace("</s>", "").replace("<pad>", "").strip()
                querys = [query]
            print("curr_query: {}".format(querys))
...
def main():
    ...
    ## Required parameters
    parser.add_argument("--classifier_type", default=None, type=str, required=True)
    parser.add_argument("--classifier_name_or_path", default=None, type=str, required=True)
    parser.add_argument("--generator_type", default=None, type=str, required=True)
    parser.add_argument("--generator_tokenizer", default=None, type=str, required=True)
    # parser.add_argument("--generator_name_or_path", default=None, type=str, required=True)
    parser.add_argument("--seqgen_name_or_path", default=None, type=str, required=True)
    parser.add_argument("--paragen_name_or_path", default=None, type=str, required=True)
    ...
    parser.add_argument("--max_cls_length", default=160, type=int)
    ...
    # Load Classifier
    args.classifier_type = args.classifier_type.lower()
    classifier_config_class, classifier_model_class, classifier_tokenizer_class = CLASSIFIER_CLASSES[args.classifier_type]
    classifier_config = classifier_config_class.from_pretrained(
        args.classifier_name_or_path,
        cache_dir=None
    )
    classifier_tokenizer = classifier_tokenizer_class.from_pretrained(
        args.classifier_name_or_path,
        do_lower_case=args.do_lower_case,
        cache_dir=None
    )
    classifier = classifier_model_class.from_pretrained(
        args.classifier_name_or_path,
        from_tf=bool('.ckpt' in args.classifier_name_or_path),
    ...
    # Load Generator
    args.generator_type = args.generator_type.lower()
    generator_config_class, generator_model_class, generator_tokenizer_class = GENERATOR_CLASSES[args.generator_type]
    seqgen_config = generator_config_class.from_pretrained(
        args.seqgen_name_or_path,
        cache_dir=None
    )
    paragen_config = generator_config_class.from_pretrained(
        args.paragen_name_or_path,
        cache_dir=None
    )
    generator_tokenizer = generator_tokenizer_class.from_pretrained(
        args.generator_tokenizer,
        do_lower_case=args.do_lower_case,
        cache_dir=None
    )
    sequence_generator = generator_model_class.from_pretrained(
        args.seqgen_name_or_path,
        from_tf=bool('.ckpt' in args.seqgen_name_or_path),
        config=seqgen_config,
        cache_dir=None
    )
    parallel_generator = generator_model_class.from_pretrained(
        args.paragen_name_or_path,
        from_tf=bool('.ckpt' in args.paragen_name_or_path),
        config=paragen_config,
        cache_dir=None
    )
    sequence_generator.to(args.device)
    parallel_generator.to(args.device)
